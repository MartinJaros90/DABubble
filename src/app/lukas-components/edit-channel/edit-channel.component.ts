...
export class EditChannelComponent {
  ...
  constructor() {
    ...
    this.mediaQueryListener = (event: MediaQueryListEvent) => { this.isMobile = event.matches };
  }
  ...
  openEditChannelName(el: HTMLElement, el2: HTMLElement) {
    ...
    this.newChannelName = '';
    setTimeout(() => { this.editChannelName = true }, 120);
    setTimeout(() => { document.getElementById('input1')?.focus() }, 170);
  ...
  channelDescriptionEditClick() {
    ...
    if (!this.editChannelDescription) {
      ...
      elements.forEach((element, index) => { element.classList.remove(this.classesForMovableElements[index]) });
    }
  }
  ...
  openEditChannelDescription(el: HTMLElement, el2: HTMLElement, el3: HTMLElement, el4: HTMLElement) {
    ...
    setTimeout(() => { this.editChannelDescription = true }, 120);
    setTimeout(() => { document.getElementById('input2')?.focus(); }, 170);
    const elements = [el, el2, el3, el4];
    elements.forEach((element, index) => { element.classList.add(this.classesForMovableElements[index]) });
  }
